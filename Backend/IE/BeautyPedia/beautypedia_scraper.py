...
def get_product_by_category(category_list, excluded_category):
    for i,item in enumerate(category_list):
        ...
        for link,name in zip(product_links, product_names):
            record = {'product_links':link, 'product_names':name, 'category':'Makeup Tools','sub_category':category}
    ...
    for i in tqdm(range(4082,len(mega_data))):
        ...
        if usr_review_summary_content is not None:
            ...
            if usr_cons_block is not None:
                ...
                usr_cons_summary = [[p.text for p in r.find_all('span') if r.find_all('span') is not None] for r in usr_cons_summary]
            if usr_cons_summary == [[]]:
                usr_cons_summary = None
            
            usr_review_summary = {'usr_review_count':usr_review_count,'usr_pros_summary':usr_pros_summary,'usr_cons_summary':usr_cons_summary}
        ...
        while next_flag:
            ...
            if usr_review_details_content is not None:
                ...
                for review in usr_reviews_block:
                    ...
                    review_record = {'review_title':review_title,'review_rating':review_rating, 'review_content':review_content,'review_cons':review_cons,'review_pros':review_pros,'review_best_for':review_best_for,'usr_routine_time':usr_routine_time,'usr_def_tag':usr_def_tag,'usr_age':usr_age,'usr_skin_type':usr_skin_type}
                ...
                try:
                    next_btn = browser.find_element_by_xpath("//a[@class='pr-rd-pagination-btn' and @aria-label='Next']")
        ...
        if len(reviews) ==0:
            reviews = None 
        usr_review_details = {'positive_reviews':positive_reviews,'negative_reviews':negative_reviews,'reviews':reviews}

        mega_data[i]['user_reviews'] = {'usr_review_summary':usr_review_summary,'usr_review_details':usr_review_details}
