...
def get_avg_counter_score(item_data, age, skin, skin_color, hair, hair_color, eye):
    ...
    # if not counter_sum <= 4 * review_cnt:
    #     print(counter_sum, 4 * review_cnt)
    #     print(item_data)
    ...
    norm_counter_score = (age_score + skin_score + skin_color_score + hair_score +
                          hair_color_score + eye_score) / counter_sum
...
def reco(cleaned_input, request):
    ...
    for item in cleaned_input:
        ...
        score = avg_counter_score * sum([weights[i] * each_score_list[i] for i in range(len(each_score_list))]) / len(each_score_list)
