...
ewg_data = rltk.Dataset(reader=rltk.JsonLinesReader('agg_ewg_data.jl'), record_class=EWG, adapter=rltk.MemoryKeyValueAdapter())
mua_data = rltk.Dataset(reader=rltk.JsonLinesReader('mua_meta_data.jl'), record_class=MakeupAlley, adapter=rltk.MemoryKeyValueAdapter())
bp_data = rltk.Dataset(reader=rltk.JsonLinesReader('beautypedia.jl'), record_class=BeautyPedia, adapter=rltk.MemoryKeyValueAdapter())
...
def rule_based_method(r_ewg,r_mua):
    ...
    if rltk.levenshtein_distance(r_ewg.brand.lower(),r_mua.brand.lower()) <= min(len(r_ewg.brand),len(r_mua.brand))/3:
        brand_score = 1
    
    category_score = 0
    cor_cat_dict = {'Bronzer/Highlighter':['Face','Cheeks'],'Concealer':['Concealer'],'Facial Powder':['Pressed Powder','Loose Powder'],'Foundation':['Foundation'],'Makeup Primer':['Face Primer','Color Corrector'],'Makeup Remover':['Other Face','Other Eyes','Other Lips','Other Cheeks'],'Brow Liner':['Eyebrows'],'Eye Liner':['Eyeliner'],'Eye Makeup Remover':['Other Eyes'],'Eye Shadow':['Eyeshadow','Eye Palettes'],'Mascara':['Mascara'],'Eyelash Glue':['False Lashes','Other Eyes'],'Other Eye Makeup':['False Lashes','Other Eyes','Eyeshadow'],'Lip Balm':['Lip Balm & Treatment'],'Lip balm with SPF':['Lip Balm & Treatment'],'Lip Gloss':['Lip Gloss'],'Lip Liner':['Lip Liner'],'Lip Plumper':['Lip Plumper'],'Lipstick':['Lipstick'],'Body Art':[''],'Glitter':['']}
...
for r_ewg,r_mua in pairs:
    ...
    if result:
        if r_mua.id in candidates:
            candidates[r_mua.id].append([r_ewg.id,r_ewg.name,r_ewg.colors,r_ewg.brand,r_mua.id,r_mua.name,r_mua.brand,confidence]) 
        else:
            candidates[r_mua.id] = [[r_ewg.id,r_ewg.name,r_ewg.colors,r_ewg.brand,r_mua.id,r_mua.name,r_mua.brand,confidence]]
...
def rule_based_method(r_mua,r_bp):
    ...
    category_score = 0
    cor_cat_dict = {'Eyebrow Makeup':['Eyebrows'],'Eyelash Primer & Treatment':['Eye Primer','Lash Serum','False Lashes','Other Eyes','Mascara'],'Eyeliner':['Eyeliner'],'Eyeshadow':['Eyeshadow'],'Eyeshadow Palette':['Eyeshadow','Eye Palettes'],'Eyeshadow Primer & Base':['Eye Primer','Other Eyes'],'Mascara':['Mascara'],'Waterproof Mascara':['Mascara'],'BB & CC Cream':['BB & CC Cream'],'Blush':['Blush','Blush Palettes','Other Cheeks'],'Bronzer':['Bronzer','Other Cheeks'],'Concealer & Corrector':['Color Corrector','Concealer'],'Contour':['Contour'],'Face Powder':['Loose Powder','Pressed Powder','Face Palettes','Other Face'],'Foundation Primer':['Face Primer','Other Foundation','Other Face'],'Foundation With Sunscreen':['Foundation'],'Foundation Without Sunscreen':['Foundation'],'Highlighter':['Highlighter'],'Setting Spray':['Setting Spray'],'Tinted Moisturizer':['Tinted Moisturizer'],'Lip Gloss':['Lip Gloss','Lip Plumper','Other Lips','Lip Balm & Treatment'],'Lip Liner':['Lip Liner'],'Lipstick':['Lipstick','Other Lips','Lip Palettes']}
